...
class Pop_Cache_File extends Pop_Cache 
{
	private $cache_dir;
	private $filename;
	...
	{
		if (!$this->cache_dir) {
			throw new Pop_Cache_Exception("no cache directory specified");
		}
		if (!file_exists($this->cache_dir)) {
			if (!mkdir($this->cache_dir,0770,true)) {
				throw new Pop_Cache_Exception("cannot create directory: ".$this->cache_dir);
			}
		}
	}
	...
	public function expunge() 
	{
		...
		if ('cache' != array_pop(explode('/',$this->cache_dir))) {
			throw new Pop_Cache_Exception("can only expunge contents of directory called 'cache'");
		}
		$i = 0;
	...
	public function getData($filename,$ttl=0)
	{
		...
		//Pop_Log::debug(LOG_FILE,'cache HIT!!! '.$filepath);
